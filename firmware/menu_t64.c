...
static u8 t64_dir(T64_STATE *state)
{
    ...
    {
        selected_element = cfg_file.img.element % MAX_ELEMENTS_PAGE;
        u8 page = cfg_file.img.element / MAX_ELEMENTS_PAGE;
...
{
    cfg_file.img.element = ELEMENT_NOT_SELECTED;    // Do not auto open T64 again
...
static bool t64_find_element(T64_STATE *state, u16 element)
{
    if (element == 1)
    {
...
static void t64_sanitize_filename(char *dest, T64_STATE *state)
{
    d64_sanitize_name_pad(dest, state->image.entry.filename, 16);
    dest[16] = 0;
}
...
static u8 t64_select(T64_STATE *state, u8 flags, u8 element_no)
{
    ...
    bool found = t64_find_element(state, element);
    if (!found)
    {
        // File not not found
        if (element == 1)
        {
            cfg_file.img.element = 1;
            return handle_unsupported_ex("Not Found",
                "No PRG files were found in image", cfg_file.file);
        }
    ...
    t64_sanitize_filename(scratch_buf, state);

    if (flags & SELECT_FLAG_OPTIONS)
    {
        return handle_file_options(scratch_buf, FILE_T64_PRG, element_no);
    }

    if (!t64_prg_size_valid(&state->image))
    {
        return handle_unsupported(scratch_buf);
    }

    cfg_file.img.mode = PRG_MODE_T64;
...
static u8 t64_load_first(const char *file_name)
{
    ...
    u8 cmd = t64_select(&t64_state, 0, 1);
    if (cfg_file.img.element == 1)
    {
        cfg_file.img.element = ELEMENT_NOT_SELECTED;    // don't auto open T64
    }
