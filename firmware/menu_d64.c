...
static void d64_format_diskname(char *buffer, const char *name, u8 name_len)
{
    ...
    // Entry type
    const u8 entry_len = (ELEMENT_LENGTH-11) - name_len;
                             ...
                             const char *filename, u8 type)
{
    ...
    *buffer++ = ' ';
    ...
    buffer += 16;
    d64_sanitize_name_pad(buffer, "", 11);
    buffer += 11;
...
{
    cfg_file.img.element = ELEMENT_NOT_SELECTED;    // Do not auto open D64 again
...
static u8 d64_select(D64_STATE *state, u8 flags, u8 element_no)
{
    ...
    D64_DIR_ENTRY *entry = d64_find_element(state, element);
    if (!entry)
    {
        // File not not found
        if (element == 1)
    ...
    if (flags & SELECT_FLAG_OPTIONS)
    {
        return handle_file_options(scratch_buf, FILE_D64_PRG, element_no);
    }

    if (flags & SELECT_FLAG_DELETE)
    {
        return d64_handle_delete_file(state, scratch_buf, entry);
    }
    ...
    cfg_file.img.mode = PRG_MODE_D64;
...
{
    if (!d64_open(&d64_state.image, filename))
    {
        return NULL;
    }
...
static const MENU * d64_menu_init(const char *filename)
{
    if (!d64_open_image(filename))
    {
        fail_to_read_sd();
    }
