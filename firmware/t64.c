...
static inline u16 t64_prg_size(T64_IMAGE *image)
{
    return image->entry.end_address - image->entry.start_address;
}

static bool t64_prg_size_valid(T64_IMAGE *image)
{
    u16 size = t64_prg_size(image) + 2;
    return prg_size_valid(size);
}

static size_t t64_read_prg(T64_IMAGE *image, u8 *buf, size_t buf_size)
{
    u16 prg_size = t64_prg_size(image);
